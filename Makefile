include local.mk
CC=chpl
MODULES=-M$(CHINGON_HOME)/src -M$(CDO_HOME)/src -M$(NUMSUCH_HOME)/src -M$(CDOEXTRAS_HOME)/src -M$(EPOCH_HOME)/src
...
test: $(SRCDIR)/Relch.chpl $(SRCDIR)/policies.chpl $(SRCDIR)/agents.chpl $(SRCDIR)/physics.chpl $(TESTDIR)/RelchTests.chpl
